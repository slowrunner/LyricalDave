...
loghandler = logging.FileHandler(HOME+'/logs/speak.log')
...
def say_espeak(phrase,vol=100,anytime=False):
    ...
    if (quietTime(ignore=anytime)):
        ...
        subprocess.check_output(['espeak-ng -s250 -p 99 -ven-us+f1 -a'+str(vol)+' "%s"' % phrase], stderr=subprocess.STDOUT, shell=True)
