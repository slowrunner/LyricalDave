...
bool SayService::setRequest(typename dave_interfaces::srv::Say::Request::SharedPtr & request)
{
  ...
  if (!getInput("saystring", text)) {
    RCLCPP_ERROR(node_->get_logger(), "SayService: missing required input [saystring]");
    return false;
  }
...
{
  RCLCPP_ERROR(node_->get_logger(), "SayService failed with error: %d", static_cast<int>(error));
