...
bool SetGrammar::setRequest(typename voskros::srv::SetGrammar::Request::SharedPtr & request)
{
  ...
  if (!getInput("grammar_list", list_str)) {
    RCLCPP_ERROR(node_->get_logger(), "SetGrammar: missing required input [grammar_list]");
    return false;
  }
...
{
  RCLCPP_INFO(node_->get_logger(), "Grammar set successfully");
  return BT::NodeStatus::SUCCESS;
}

BT::NodeStatus SetGrammar::onFailure(BT::ServiceNodeErrorCode error)
{
  RCLCPP_ERROR(node_->get_logger(), "SetGrammar service failed with error: %d", static_cast<int>(error));
