...
# - increase "food" distance from 100mm to 200mm to allow for slow loop.
...
def motorcontrol():
        ...
        speak(f"Left {accumleft:.0f}, Right {accumright:.0f}, Speed {new_speed:.0f}")
        ## Start Commented section
        gpg.set_speed(new_speed)
        print("Speed set: ", new_speed)
        # speak(f"New speed {new_speed:.0f}")
...
### Use CNTRL-C to stop the program
    while True:
        ...
        # increase distance to prevent pushing against food
        if dist>0 and dist<200:
            print("OBSTACLE (Nose Touch)", dist)
            speak(f"Nose Touch {dist:.0f} millimeters")
