...
# Usage:  place this file (and mock easygopigo3.py) in same folder as a GoPiGo3 program
...
try:
    ...
    print("Can't import spidev. GoPiGo3 hardware support will be disabled.")
...
if hardware_connected:
    ...
    try:
        ...
        raise SystemExit(
            "SPI is not enabled. Please enable it with:\n"
...
class GoPiGoValueError(Exception):
...
class GoPiGo3:
    ...
    def __init__(self, addr = 8, detect = True, config_file_path=None):
        ...
        * Optionally disable the detection of the GoPiGo3 hardware. This can be used for debugging
        ...
        # (use 'sudo raspi-config' -> Interface Options -> SPI).
        ...
        if detect:
            ...
            if vfw.split('.')[0] != FIRMWARE_VERSION_REQUIRED.split('.')[0] or \
               vfw.split('.')[1] != FIRMWARE_VERSION_REQUIRED.split('.')[1]:
                raise FirmwareVersionError(f"GoPiGo3 firmware needs to be version {FIRMWARE_VERSION_REQUIRED} but is currently version {vfw}")
    ...
    def spi_read_32(self, MessageType):
        ...
        reply = self.spi_transfer_array(outArray)
        if(reply[3] == 0xA5):
            return (reply[4] << 24) | (reply[5] << 16) | (reply[6] << 8) | reply[7]
        raise IOError("No SPI response")
        ...
        try:
            with open(config_file_path, 'r') as json_file:
                ...
                if data['wheel-diameter'] > 0 and data['wheel-base-width'] > 0 and ticks > 0 and motor_gear_ratio > 0:
                    ...
                    raise GoPiGoValueError('positive values required')
    ...
    def get_id(self):
        ...
        reply = self.spi_transfer_array(outArray)
        if(reply[3] == 0xA5):
            return bytes(reply[4:20]).hex().upper()
        raise IOError("No SPI response")
    ...
    def get_motor_status(self, port):
        ...
        if(reply[3] == 0xA5):
            ...
            encoder = (reply[6] << 24) | (reply[7] << 16) | (reply[8] << 8) | reply[9]
            if encoder & 0x80000000:
                encoder -= 0x100000000

            dps = (reply[10] << 8) | reply[11]
            if dps & 0x8000:
                dps -= 0x10000
    ...
    def grove_i2c_start(self, port, addr, outArr, inBytes = 0):
        ...
        if inBytes > self.GROVE_I2C_LENGTH_LIMIT:
            raise RuntimeError(f"Read length error. Up to {self.GROVE_I2C_LENGTH_LIMIT} bytes can be read in a single transaction.")

        outBytes = len(outArr)
        if outBytes > self.GROVE_I2C_LENGTH_LIMIT:
            raise RuntimeError(f"Write length error. Up to {self.GROVE_I2C_LENGTH_LIMIT} bytes can be written in a single transaction.")
            ...
            if(reply[3] == 0xA5):
                if(reply[4] == self.GroveType[port_index] and reply[5] == 0):
                    ...
                    if value == 0:
                        ...
                        raise GoPiGoValueError("get_grove_value error: Object not detected within range")
                    else:
                        return value
                else:
                    raise SensorError("get_grove_value error: Invalid value")
            ...
            if(reply[3] == 0xA5):
                if(reply[4] == self.GroveType[port_index]):
                    if(reply[5] == self.GROVE_STATE.VALID_DATA):  # no error
                        ...
                        raise GoPiGoValueError("get_grove_value error: Invalid value")
                else:
    ...
    def get_grove_state(self, pin):
        ...
        if(reply[3] == 0xA5):
            if(reply[4] == self.GROVE_STATE.VALID_DATA): # no error
                ...
                raise GoPiGoValueError("get_grove_state error: Invalid value")
        else:
    ...
    def get_grove_voltage(self, pin):
        ...
        if(reply[3] == 0xA5):
            if(reply[4] == self.GROVE_STATE.VALID_DATA): # no error
                ...
                raise GoPiGoValueError("get_grove_voltage error: Invalid value")
        else:
    ...
    def get_grove_analog(self, pin):
        ...
        if(reply[3] == 0xA5):
            if(reply[4] == self.GROVE_STATE.VALID_DATA): # no error
                ...
                raise GoPiGoValueError("get_grove_analog error: Invalid value")
        else:
